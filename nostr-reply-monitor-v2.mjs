#!/usr/bin/env node
/**
 * Nostr Reply Monitor V2 - Subscription-Based
 * 
 * Uses subscriptions instead of querySync for better relay compatibility
 */
...
function log(message) {
  ...
  const logMessage = `[${timestamp}] [REPLY-MONITOR-V2] ${message}\n`
...
function loadIdentity() {
  try {
    ...
    return { publicKey: pubkeyMatch[1] }
  } catch (err) {
...
/**
 * Query with subscription (more reliable than querySync)
 */
async function queryWithSubscription(pool, relays, filter, timeout = QUERY_TIMEOUT) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      log(`   Timeout reached, collected ${events.length} events`)
      resolve(events)
    }, timeout)
    
    // Subscribe to each relay individually
    relays.forEach(relay => {
      pool.sub([relay], [filter]).on('event', event => {
...
async function monitorReplies() {
  log('🔍 Starting reply monitoring (V2 - subscription-based)...')
  ...
  log(`   Using subscription-based query...`)
  
  const myPosts = await queryWithSubscription(pool, RELAYS, {
    kinds: [1],
    authors: [myPubkey],
    since: since
  })
  ...
  const replies = await queryWithSubscription(pool, RELAYS, {
    kinds: [1],
    '#e': myPostIds,
    since: since
  })
