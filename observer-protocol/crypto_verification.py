...
def verify_signature_simple(message: bytes, signature_hex: str, public_key_hex: str) -> bool:
    ...
    Supports both compressed (33 bytes) and uncompressed (65 bytes) public keys.
    """
    try:
        # Decode signature
        sig_bytes = bytes.fromhex(signature_hex)
        print(f"DEBUG: Signature bytes: {len(sig_bytes)}")
        
        # Decode public key
        public_key_bytes = bytes.fromhex(public_key_hex)
        print(f"DEBUG: Public key bytes: {len(public_key_bytes)}, first byte: {hex(public_key_bytes[0]) if public_key_bytes else 'N/A'}")
        ...
        if len(public_key_bytes) == 65 and public_key_bytes[0] == 0x04:
            ...
            print("DEBUG: Loaded uncompressed public key")
            ...
            # Use the cryptography library's from_encoded_point method
            public_key = ec_module.EllipticCurvePublicKey.from_encoded_point(
                ec_module.SECP256K1(),
                public_key_bytes
            )
            print("DEBUG: Loaded compressed public key")
        else:
            print(f"DEBUG: Unsupported public key format: {len(public_key_bytes)} bytes")
            return False
        ...
        if len(sig_bytes) == 64:
            ...
            print(f"DEBUG: Parsed raw signature, DER length: {len(signature_der)}")
        elif 68 <= len(sig_bytes) <= 72:
            # Already DER format
            signature_der = sig_bytes
            print(f"DEBUG: Using DER signature, length: {len(signature_der)}")
        else:
            print(f"DEBUG: Unsupported signature format: {len(sig_bytes)} bytes")
            return False
        ...
        print(f"DEBUG: InvalidSignature error: {e}")
        return False
    except Exception as e:
