#!/bin/bash
# ArcadiaB "El Peso Se Derrite" Video Assembly Pipeline - FIXED VERSION
...
# Step 3: Add text overlays - FIXED with proper scaling
echo "🔄 Step 3: Adding text overlays..."
OVERLAY_VIDEO="$OUTPUT_DIR/with_overlays.mp4"

# Scale overlays to match video resolution and use fade transitions
ffmpeg -y -i "$CONCAT_VIDEO" \
    ...
    -filter_complex "
        [1:v]scale=1080:1920[ol1];
        [2:v]scale=1080:1920[ol2];
        [3:v]scale=1080:1920[ol3];
        [4:v]scale=1080:1920[ol4];
        [5:v]scale=1080:1920[ol5];
        [6:v]scale=1080:1920[ol6];
        [7:v]scale=1080:1920[ol7];
        [8:v]scale=1080:1920[ol8];
        [9:v]scale=1080:1920[ol9];
        [0:v][ol1]overlay=0:0:enable='between(t,0,3)'[v1];
        [v1][ol2]overlay=0:0:enable='between(t,3,6)'[v2];
        [v2][ol3]overlay=0:0:enable='between(t,6,9)'[v3];
        [v3][ol4]overlay=0:0:enable='between(t,9,12)'[v4];
        [v4][ol5]overlay=0:0:enable='between(t,12,15)'[v5];
        [v5][ol6]overlay=0:0:enable='between(t,15,18)'[v6];
        [v6][ol7]overlay=0:0:enable='between(t,18,21)'[v7];
        [v7][ol8]overlay=0:0:enable='between(t,21,24)'[v8];
        [v8][ol9]overlay=0:0:enable='between(t,24,27)'[v9]
    " \
    -c:v $VIDEO_CODEC -crf $CRF -preset fast \
...
# Step 4: Add end card and music
echo "🔄 Step 4: Adding end card and music..."
