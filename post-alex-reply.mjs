#!/usr/bin/env node
/**
 * Post reply to Alex B using OAuth 1.0a
 */
...
function postReply(text, replyToId) {
  return new Promise((resolve, reject) => {
    ...
    // OAuth params include body params for signature
    const oauthParams = {};
    ...
    const options = {
      ...
      method: 'POST',
      headers: {
        'Authorization': buildAuthHeader('POST', url, oauthParams),
...
const replyText = "Lightning IS programmable — HTLCs are literally conditional payments (hashlock + timelock = escrow). BOLT12 enables pull payments. Inbound liquidity is an LSP/swap problem, not protocol. Different tradeoffs, not mutually exclusive.";
const replyToId = '2026871802463662552';

postReply(replyText, replyToId)
  .then((response) => {
    console.log('✅ Reply posted to Alex B!');
